...
Seahorse 首版应聚焦一个最小且完整的记忆引擎闭环：`ingest -> recall -> forget -> rebuild`
。首版目标不是实现完整“仿脑认知系统”，而是交付一个可写入、可召回、可删除、可重建、可观测、可测试的工程化基础版本。
在补齐 schema 约束、状态机、作业跟踪和发布门槛后，本稿可以直接作为 MVP 实施基线。

### 1.2 首版定位

Seahorse MVP 是一个以 SQLite 为事实源、以向量检索为主召回路径、以 Tag 提取与规范化作为辅助语义层、通过基础 REST API
暴露能力的本地/服务化记忆引擎。
...
当前仓库已有一份战略级设计稿 `docs/design-all.md`，其中包含大量长期愿景、仿脑叙事、认知增强算法、平台化接口和生态扩展设想。这些内容对项目方向有价值，但不适合作为首版工程实施依据。
...
| rebuild 作业状态跟踪                | MVP 必须项 | 支撑异步管理接口与故障恢复                   |
...
- `connectome`
- `neuron_states`
- 完整 `namespaces` 体系（但建议预留 `namespace` 字段，MVP 固定为 `default`）
...
- 若未启用 `embedding_cache`，MVP 不要求持久化 `embedding_id`；rebuild 默认基于 `chunk_text` 重新生成 embedding。
- `index_status` 建议至少覆盖 `pending`、`ready`、`failed`、`deleted` 四种状态。
- 请求级 `metadata` 默认落到 `files.metadata_json`；只有 chunker 产出分片级元数据时才写入 `chunks.metadata_json`。
...
- `chunks(file_id, chunk_index)` 必须唯一，保证 chunk 顺序稳定且可重建。
- `tags(namespace, normalized_name)` 必须唯一，保证标签归一化结果稳定。
- `chunk_tags(chunk_id, tag_id)` 必须唯一，避免重复关系。
- `chunks(namespace, is_deleted, index_status)`、`repair_queue(status)`、`maintenance_jobs(status)` 必须建立索引，支撑
  recall、repair 和 job polling。
- `files(namespace, file_hash)` 建议建立索引，用于重复写入判定；若默认策略固定为拒绝重复，可提升为唯一索引。
- `schema_meta` 至少跟踪 `schema_version`、`index_version`、`embedding_model_id`、`embedding_dimension`。
...
| `source.filename`    | 可选，长度 `1..255`                    | 超限返回 `INVALID_INPUT`   |
| `tags`               | 可选，最多 `32` 个                      | 超限返回 `INVALID_INPUT`   |
| 单个 `tag`             | 长度 `1..64` 字符                     | 超限返回 `INVALID_INPUT`   |
| `metadata`           | 可选，建议限制为平铺 JSON 对象，序列化后不超过 `16KB` | 非法结构返回 `INVALID_INPUT` |
| `namespace`          | 可选，MVP 阶段仅接受 `default`            | 非法值返回 `INVALID_INPUT`  |
| `options.chunk_mode` | 可选，仅允许 `fixed`，省略等同 `fixed`        | 非法值返回 `INVALID_INPUT`  |
| `options.auto_tag`   | 可选，布尔值，默认 `false`                 | 非法类型返回 `INVALID_INPUT` |
| `options.dedup_mode` | 可选，取值 `reject / upsert / allow`   | 非法值返回 `INVALID_INPUT`  |
| `timeout_ms`         | 仅 `/recall` 支持，可选正整数超时预算          | 非法类型返回 `INVALID_INPUT` |

首版应明确拒绝无法解析的非法文本输入、超长字段和不受控 metadata 结构，避免存储污染和接口歧义。

#### 7.2.1.b 幂等与重复写入策略

- 服务端默认基于规范化后的 `file_hash` 判定重复写入。
- `dedup_mode = reject`：命中重复内容时直接返回已有结果，不重复写入。
- `dedup_mode = upsert`：对同一 `file_hash` 的现存有效 chunks 执行软删除，再写入新版本。
- `dedup_mode = allow`：允许重复写入，由调用方承担重复召回风险。
- 即使调用方未提供 `source.filename`，也必须基于内容计算 `file_hash`，保证重试场景具备幂等基础。
...
- 向量索引更新可在事务提交后执行。
- 若向量索引失败，不回滚已提交数据，但必须把 `files.ingest_status` 标记为 `partial`、把相关 `chunks.index_status` 标记为
  `failed`，并写入 `repair_queue`。
- 仅当所有 chunk 对应索引节点可见时，才将 `files.ingest_status` 置为 `ready`。
...
- 超过最大重试次数后进入 `deadletter`

#### 7.5.4 rebuild 作业模型

- `POST /admin/rebuild` 必须创建持久化作业记录到 `maintenance_jobs`，避免服务重启后丢失状态。
- `maintenance_jobs.status` 建议使用 `queued`、`running`、`succeeded`、`failed`、`cancelled`。
- 同一 `namespace` 在任一时刻只允许一个激活中的 `rebuild` 作业；重复请求默认返回当前运行中的 `job_id`，仅 `force = true`
  时允许替换策略。
- `progress` 至少应反映 `scanned_chunks / total_chunks` 或阶段化进度，便于运维判断卡点。
...
建议首版统一使用以下错误码，并绑定明确 HTTP 语义：

| 错误码                 | HTTP 状态   | 是否可重试 | 说明                            |
|---------------------|-----------|-------|-------------------------------|
| `INVALID_INPUT`     | `400/404` | 否     | 参数校验失败，或资源/作业 id 不存在           |
| `EMBEDDING_FAILED`  | `503`     | 是     | 上游 embedding provider 失败，不含 provider timeout |
| `STORAGE_ERROR`     | `500`     | 视场景而定 | SQLite 事务失败、迁移失败、写盘失败         |
| `INDEX_UNAVAILABLE` | `503`     | 是     | 当前索引不可用且无法返回有效降级结果            |
| `REBUILD_FAILED`    | `500`     | 是     | rebuild 提交或准备阶段失败；异步执行失败主要通过 job.status=failed 暴露 |
| `TIMEOUT`           | `504`     | 是     | 请求超时                          |
...
- `files.ingest_status`：`pending_index` / `ready` / `partial` / `deleted`
- `chunks.index_status`：`pending` / `ready` / `failed` / `deleted`
- `schema_meta.index_state`：`ready` / `rebuilding` / `degraded` / `unavailable`
- `health.status`：`ok` / `degraded` / `failed`

#### 7.9.2 状态切换原则

- 新写入数据先进入 `pending_index`，索引完成后切到 `ready`。
- 索引失败时只允许进入 `partial` 或 `failed`，不得伪装成成功。
- rebuild 期间 `schema_meta.index_state = rebuilding`，若主索引仍可用则 `health.status = degraded`；若不可用则返回
  `failed` 或 `INDEX_UNAVAILABLE`。
- 删除行为必须同时影响可见性与索引状态，避免 tombstone 数据继续进入召回结果。

### 7.10 统一响应契约与 REST API 最小契约
...
- `tags?: string[]`
- `metadata?: object`
- `options?: { chunk_mode?: "fixed", auto_tag?: boolean, dedup_mode?: "reject" | "upsert" | "allow" }`
  - `chunk_mode` 省略等同 `fixed`，当前构建不支持其它模式
  - `auto_tag` 为规则型启发式自动打标，不依赖模型
...
- `chunk_ids`
- `ingest_status`
- `index_status`（`ready` / `pending_repair`）
- `warnings`（可选）
...
- `mode?: "basic"`
- `timeout_ms?: number`
  - 仅作为单次召回的超时预算，命中返回 `504 TIMEOUT`

响应建议字段：

- `results: RecallResultItem[]`
- `metadata: { top_k, latency_ms, degraded, degraded_reason?, result_count, index_state }`
...
首版正式契约只保留 `soft`。`hard delete` 不进入当前 MVP 的正式 API 契约；如后续需要，应以单独管理接口或后续版本另行定义。
...
- `/health` 保持为人工巡检接口，即使内部健康状态为 `failed` 也返回 `200`

#### `GET /ready`

返回说明：

- 面向平台 readiness probe
- 当服务仍可提供 MVP 能力时返回 `200`
- 当内部健康状态为 `failed` 或健康快照不可获取时返回 `503`
...
- 面向平台 liveness probe
- 仅表示进程存活，不依赖数据库、索引或 repair/rebuild 队列状态
...
- 正式契约采用默认路径 `/metrics`
- 仅当 `observability.enable_metrics=true` 时挂载该接口；默认配置为开启
- 若运行时通过 `observability.metrics_path` 覆盖路径，则同一份 Prometheus 文本格式响应暴露在配置路径上
- `Content-Type: text/plain; version=0.0.4`
- 指标至少覆盖 HTTP 请求量、错误量、最大延迟、repair queue 状态分布、rebuild job 状态分布、索引状态和健康状态
...
- schema 迁移、冷启动、首次 rebuild 必须经过自动化验证。
- 至少完成一轮 `ingest -> recall -> forget -> rebuild -> recall` 端到端回归。
- 在 `1 万` chunk 基线数据集上满足 `recall P95 < 300ms`，且无 P0/P1 数据一致性缺陷。
- API 契约测试必须固定请求响应结构，避免字段漂移。
- `repair_queue` 与 `maintenance_jobs` 在异常注入场景下必须能正确反映失败与恢复状态。
...
| 接口任务 | ingest/recall/forget/rebuild/stats/health/metrics 契约、错误码、输入校验                    | API 文档或 OpenAPI | 请求响应结构稳定、错误码可复用                            |
...
执行拆解清单见 `docs/mvp-issue-breakdown.md`，建议直接据此创建仓库 issue 或项目看板卡片。
