...
|---------------------|-----------|-------|-------------------------------|
| `INVALID_INPUT`     | `400`     | 否     | 参数校验失败、非法 metadata、超长输入       |
| `EMBEDDING_FAILED`  | `502/503` | 是     | 上游 embedding provider 失败或超时   |
| `STORAGE_ERROR`     | `500`     | 视场景而定 | SQLite 事务失败、迁移失败、写盘失败         |
| `INDEX_UNAVAILABLE` | `503`     | 是     | 当前索引不可用且无法返回有效降级结果            |
| `REBUILD_FAILED`    | `500`     | 是     | rebuild 作业失败                  |
| `TIMEOUT`           | `504`     | 是     | 请求超时                          |
| `PARTIAL_RESULT`    | `200`     | 否     | 非致命降级，结果可用但不完整，应写入响应 metadata |
...
- `tags?: string[]`
- `metadata?: object`
- `options?: { chunk_mode?: string, auto_tag?: boolean, dedup_mode?: "reject" | "upsert" | "allow" }`
...
- 指标至少覆盖 HTTP 请求量、错误量、最大延迟、索引状态和健康状态
