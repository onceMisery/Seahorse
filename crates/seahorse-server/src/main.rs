use seahorse_server::{
    app::build_app_with_observability,
    config::load_server_config_default,
    state::AppState,
};
use tracing::info;
...
async fn main() {
    ...
    let state =
        AppState::new_with_config(&config).expect("failed to initialize seahorse application state");
